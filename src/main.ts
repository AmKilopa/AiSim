...
import { createWorld, addCountry, spawnUnitsForCountry, generateResources } from './core/World';
...
generateResources(world, config.resourceCount);
...
inputManager.onLeftClick = (worldPos: Vec2) => {
  if (placingCountry) {
    ...
    // Выбор юнита (рабочий, женщина, мужчина, военный...)
...
function updateUnitInfo() {
  ...
  unitInfoDiv.innerHTML = `
    <div style="margin-bottom: 10px; padding-bottom: 10px; border-bottom: 1px solid #333;">
      <strong style="color:${selectedUnit.type === UnitType.MILITARY ? '#00ff00' : (selectedUnit.isWorker ? '#ffaa33' : (selectedUnit.gender==='male'?'#3388ff':'#ff88ee'))}">${selectedUnit.gender==='male'?'Мужчина':'Женщина'} ${selectedUnit.isWorker?'Рабочий':''} ${selectedUnit.type===UnitType.MILITARY?'Военный':''}</strong>
    </div>
    ...
    <div><strong>Возраст:</strong> ${selectedUnit.age.toFixed(1)} лет</div>
    ...
    <div><strong>Накопленные ресурсы:</strong> ${selectedUnit.resources}</div>
  `;
}
function renderLoop() {
